...
describe("Intimidate", function() {
  ...
  it("does not throw an error if alternative options are passed in", function() {
    assert.doesNotThrow(function() {
      // eslint-disable-next-line no-unused-vars
      var client = new Intimidate(
        { accessKeyId: 1, secretAccessKey: 1, bucket: 1 },
        noopKnox
      );
    });
  });
  ...
  var failKnox = {
    createClient: function() {
      return {
        put: function(_put, _destination) {
          return {
            eventHandlers: {},
  ...
  describe("upload", function() {
    it("tries uploading to s3 until maxRetries tries or it gets the hose again", function(
      done
    ) {
      var client = new Intimidate(
        { key: 1, secret: 1, bucket: 1, backoffInterval: 1, maxRetries: 4 },
        failKnox
      );
      client.upload(
        path.join(__dirname, "fakeFile.txt"),
        "destination",
        function(err, res, timesRetried) {
          assert(err);
          assert(res == null);
    ...
    it("tries uploading to s3 until maxRetries tries or it gets the hose again: passing headers", function(
      done
    ) {
      var client = new Intimidate(
        { key: 1, secret: 1, bucket: 1, backoffInterval: 1, maxRetries: 4 },
    ...
    it("calls the callback with a response object if the request succeeds", function(
      done
    ) {
      var client = new Intimidate(
        { key: 1, secret: 1, bucket: 1 },
        successKnox
      );
      client.upload(
        path.join(__dirname, "fakeFile.txt"),
  ...
  it("calls the callback with a response object if the request succeeds: passing headers", function(
    done
  ) {
  ...
  describe("uploadBuffer", function() {
    it("uploads a buffer", function(done) {
      var data = makeBuffer("Shall i compare thee to a summer's day?");
      var headers = {
  ...
  describe("uploadFiles", function() {
    it("calls the callback with a array response object if the request succeeds", function(
      done
    ) {
      var client = new Intimidate(
        { key: 1, secret: 1, bucket: 1 },
  ...
  describe("uploadBuffers", function() {
    it("calls the callback with a array response object if the request succeeds", function(
      ...
      var buffers = [
        {
          data: makeBuffer("Shall I compare thee to a summer's day?"),
          dest: "destination"
        },
        {
          data: makeBuffer(
            "When you need those uploads to back off, use intimidate"
          ),
          dest: "another_destination"
        }
      ];
...
function makeBuffer(str) {
  // if we have Buffer.from, use it since new Buffer is deprecated in node 6
  if (Buffer.from) {
